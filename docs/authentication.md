...
Go Feather Route uses bearer authentication at the gateway boundary when
`GOFEATHERROUTE_API_KEY` is configured.
...
and chat completions require the gateway key when one is configured.
