...
Fallbacks are configured per exact `model_list` alias and are evaluated in
order. A successful fallback response is marked in the usage event with
`fallback_used`; a provider that reaches the failure threshold is temporarily
cooled down. Cost rates are configured on the model route as USD per million
input/output tokens. Provider-reported cost takes precedence when a provider
adapter exposes it.
